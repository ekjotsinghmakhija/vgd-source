...
vgd is a distributed AI inference system that connects multiple devices into a cluster. It enables running large language models across multiple machines using MLX as the inference backend and libp2p for peer-to-peer networking.

## Build & Run Commands

```bash
# Build the dashboard (required before running vgd)
cd dashboard && npm install && npm run build && cd ..

# Run vgd (starts both master and worker with API at http://localhost:52415)
uv run vgd

# Run with verbose logging
uv run vgd -v   # or -vv for more verbose
...
uv run pytest src/vgd/shared/tests/test_election.py

# Run a specific test function
uv run pytest src/vgd/shared/tests/test_election.py::test_function_name
...
### Node Composition
A single vgd `Node` (src/vgd/main.py) runs multiple components:
- **Router**: libp2p-based pub/sub messaging via Rust bindings (vgd_pyo3_bindings)
...
### Message Flow
Components communicate via typed pub/sub topics (src/vgd/routing/topics.py):
...
- `State` (src/vgd/shared/types/state.py): Immutable state object
- `apply()` (src/vgd/shared/apply.py): Pure function that applies events to state
- Master indexes events and broadcasts; workers apply indexed events

### Key Type Hierarchy
- `src/vgd/shared/types/`: Pydantic models for all shared types
...
- `vgd_pyo3_bindings`: PyO3 bindings exposing Rust to Python
...
Tests use pytest-asyncio with `asyncio_mode = "auto"`. Tests are in `tests/` subdirectories alongside the code they test. The `VGD_TESTS=1` env var is set during tests.
