...
struct ContentView: View {
    @EnvironmentObject private var controller: VgdProcessController
    ...
    private var shouldShowLocalNetworkWarning: Bool {
        // Show warning if local network is not working and VGD is running.
    ...
    private func showUninstallConfirmationAlert() {
        let alert = NSAlert()
        alert.messageText = "Uninstall VGD"
        alert.informativeText = """
            This will remove VGD and all its system components:

            • Network configuration daemon
            • Launch at login registration
            • VGD network location
    ...
    private var buildTag: String {
        Bundle.main.infoDictionary?["VGDBuildTag"] as? String ?? "unknown"
    }

    private var buildCommit: String {
        Bundle.main.infoDictionary?["VGDBuildCommit"] as? String ?? "unknown"
    }
}
