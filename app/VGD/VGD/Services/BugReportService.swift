...
struct BugReportService {
    ...
    func sendReport(
        ...
        return BugReportOutcome(
            success: true, message: "Bug Report sent. Thank you for helping to improve VGD 1.0.")
    }
    ...
    {
        guard
            let endpointString = bundle.infoDictionary?["VGDBugReportPresignedUrlEndpoint"]
                as? String
        else {
    ...
    private func readLog() -> Data? {
        let logURL = URL(fileURLWithPath: NSHomeDirectory())
            .appendingPathComponent(".vgd")
            .appendingPathComponent("vgd.log")
    ...
    private func makeReportJson(
        ...
        let payload: [String: Any] = [
            ...
            "system": system,
            "vgd_version": vgd.version as Any,
            "vgd_commit": vgd.commit as Any,
    ...
    private func readVgdMetadata(bundle: Bundle = .main) -> (version: String?, commit: String?) {
        let info = bundle.infoDictionary ?? [:]
        let tag = info["VGDBuildTag"] as? String
        let short = info["CFBundleShortVersionString"] as? String
        let version = [tag, short]
            .compactMap { $0?.trimmingCharacters(in: .whitespacesAndNewlines) }
            .first { !$0.isEmpty }
        let commit = (info["VGDBuildCommit"] as? String)?
