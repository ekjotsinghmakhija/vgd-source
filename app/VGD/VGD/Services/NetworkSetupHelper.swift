...
enum NetworkSetupHelper {
    private static let logger = Logger(subsystem: "io.vgd.VGD", category: "NetworkSetup")
    private static let daemonLabel = "io.vgd.networksetup"
    private static let scriptDestination =
        "/Library/Application Support/VGD/disable_bridge.sh"
    private static let plistDestination = "/Library/LaunchDaemons/io.vgd.networksetup.plist"
    ...
    /// Removes all VGD network setup components from the system.
    ...
    static func uninstall() throws {
        ...
        logger.info("VGD network setup components removed successfully")
    }

    /// Checks if there are any VGD network components installed that need cleanup
    ...
    private static func makeUninstallScript() -> String {
        ...
        networksetup -listlocations | grep -q '^vgd$' && {
        ...
        echo "VGD network components removed successfully"
        """
    }
