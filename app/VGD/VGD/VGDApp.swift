...
@main
struct VGDApp: App {
    @StateObject private var controller: VgdProcessController
    ...
    init() {
        let controller = VgdProcessController()
        ...
        // Check local network access BEFORE launching vgd
        localNetwork.check()
    ...
    private func enableLaunchAtLoginIfNeeded() {
        ...
        do {
            ...
            Logger().error(
                "Failed to register VGD for launch at login: \(error.localizedDescription)")
        }
    }
}

/// Helper for managing VGD's launch-at-login registration
enum LaunchAtLoginHelper {
    private static let logger = Logger(subsystem: "io.vgd.VGD", category: "LaunchAtLogin")

    /// Unregisters VGD from launching at login
    static func disable() {
        guard SMAppService.mainApp.status == .enabled else { return }
        do {
            try SMAppService.mainApp.unregister()
            logger.info("Unregistered VGD from launch at login")
        } catch {
            logger.error(
                "Failed to unregister VGD from launch at login: \(error.localizedDescription, privacy: .public)"
            )
        }
    }
}
...
final class SparkleUpdater: NSObject, ObservableObject {
    ...
    private let delegateProxy: VgdUpdaterDelegate
    private let notificationDelegate = VgdNotificationDelegate()
    private var periodicCheckTask: Task<Void, Never>?

    init(processController: VgdProcessController) {
        let proxy = VgdUpdaterDelegate(processController: processController)
        delegateProxy = proxy
...
private final class VgdUpdaterDelegate: NSObject, SPUUpdaterDelegate {
    private weak var processController: VgdProcessController?
    ...
    nonisolated func updater(_ updater: SPUUpdater, didFindValidUpdate item: SUAppcastItem) {
        showNotification(
            ...
            body: "VGD \(item.displayVersionString) is ready to install."
        )
    }
    ...
    private func showNotification(title: String, body: String) {
        ...
        let request = UNNotificationRequest(
            identifier: "vgd-update-\(UUID().uuidString)",
            content: content,
            trigger: nil
        )
...
private final class VgdNotificationDelegate: NSObject, UNUserNotificationCenterDelegate {
