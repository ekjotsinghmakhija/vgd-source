...
private let customNamespaceKey = "VGDCustomNamespace"

@MainActor
final class VgdProcessController: ObservableObject {
    ...
    func launch() {
        do {
            ...
            let executableURL = runtimeURL.appendingPathComponent("vgd")
    ...
    private func resolveRuntimeDirectory() throws -> URL {
        ...
        if let override = ProcessInfo.processInfo.environment["VGD_RUNTIME_DIR"] {
        ...
        if let resourceRoot = Bundle.main.resourceURL {
            let bundled = resourceRoot.appendingPathComponent("vgd", isDirectory: true)
        ...
        let repoCandidate = URL(fileURLWithPath: fileManager.currentDirectoryPath)
            .appendingPathComponent("dist/vgd", isDirectory: true)
        if fileManager.fileExists(atPath: repoCandidate.path) {
            return repoCandidate
        }

        throw RuntimeError("Unable to locate the packaged VGD runtime.")
    }

    private func makeEnvironment(for runtimeURL: URL) -> [String: String] {
        var environment = ProcessInfo.processInfo.environment
        environment["VGD_RUNTIME_DIR"] = runtimeURL.path
        environment["VGD_LIBP2P_NAMESPACE"] = computeNamespace()
    ...
    private func buildTag() -> String {
        if let tag = Bundle.main.infoDictionary?["VGDBuildTag"] as? String, !tag.isEmpty {
            return tag
        }
