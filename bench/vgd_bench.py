...
class VgdHttpError(RuntimeError):
...
class VgdClient:
    ...
    def request_json(
        ...
        try:
            ...
            if resp.status >= 400:
                raise VgdHttpError(resp.status, resp.reason, text[:300])
...
def wait_for_instance_ready(
    client: VgdClient, instance_id: str, timeout: float = 24000.0
) -> None:
...
def wait_for_instance_gone(
    client: VgdClient, instance_id: str, timeout: float = 3.0
) -> None:
    start_time = time.time()
    while time.time() - start_time < timeout:
        try:
...
def resolve_model_short_id(client: VgdClient, model_arg: str) -> tuple[str, str]:
...
def run_one_completion(
    client: VgdClient, model_id: str, pp_hint: int, tg: int, prompt_sizer: PromptSizer
...
def main() -> int:
    ap = argparse.ArgumentParser(
        prog="vgd-bench",
        description="Benchmark vgd model throughput across placement previews.",
    )
    ap.add_argument("--host", default=os.environ.get("VGD_HOST", "localhost"))
    ap.add_argument(
        "--port", type=int, default=int(os.environ.get("VGD_PORT", "52415"))
    )
    ...
    client = VgdClient(args.host, args.port, timeout_s=args.timeout)
    ...
    if tokenizer is None:
        raise RuntimeError("[vgd-bench] tokenizer load failed")

    try:
        prompt_sizer = PromptSizer(tokenizer)
        logger.debug(f"[vgd-bench] loaded tokenizer: {full_model_id} for prompt sizer")
    except Exception:
        logger.error("[vgd-bench] tokenizer usable but prompt sizing failed")
        raise
    ...
    logger.debug(f"vgd-bench model: short_id={short_id} full_id={full_model_id}")
    ...
    for preview in selected:
        ...
        try:
            ...
            with contextlib.suppress(VgdHttpError):
