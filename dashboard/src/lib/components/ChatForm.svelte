...
>
  <div
    class="relative command-panel rounded overflow-hidden transition-all duration-200 {isDragOver
      ? 'ring-2 ring-vgd-yellow ring-opacity-50'
      : ''}"
  >
    <!-- Top accent line -->
    <div
      class="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-vgd-yellow/50 to-transparent"
    ></div>

    <!-- Drag overlay -->
    {#if isDragOver}
      <div
        class="absolute inset-0 bg-vgd-dark-gray/80 z-10 flex items-center justify-center"
      >
    ...
    {#if isEditMode && currentEditingImage}
      <div
        class="flex items-center gap-3 px-3 py-2 bg-vgd-yellow/10 border-b border-vgd-yellow/30"
      >
        <img
          src={currentEditingImage.imageDataUrl}
          alt="Source for editing"
          class="w-10 h-10 object-cover rounded border border-vgd-yellow/30"
        />
        <div class="flex-1">
          <span
            class="text-xs font-mono tracking-wider uppercase text-vgd-yellow"
            >EDITING IMAGE</span
          >
        </div>
        <button
          ...
          class="px-2 py-1 text-xs font-mono tracking-wider uppercase bg-vgd-medium-gray/30 text-vgd-light-gray border border-vgd-medium-gray/50 rounded hover:bg-vgd-medium-gray/50 hover:text-vgd-yellow transition-colors cursor-pointer"
        >
          CANCEL
        </button>
      </div>
    ...
    {#if showModelSelector && availableModels().length > 0}
      <div
        class="flex items-center justify-between gap-2 px-3 py-2 border-b border-vgd-medium-gray/30"
      >
        <div class="flex items-center gap-2 flex-1">
          <span
            class="text-xs text-vgd-light-gray uppercase tracking-wider flex-shrink-0"
            >MODEL:</span
          >
          <!-- Custom dropdown -->
          <div class="relative flex-1 max-w-xs">
            <button
              ...
              class="w-full bg-vgd-medium-gray/50 border border-vgd-yellow/30 rounded pl-3 pr-8 py-1.5 text-xs font-mono text-left tracking-wide cursor-pointer transition-all duration-200 hover:border-vgd-yellow/50 focus:outline-none focus:border-vgd-yellow/70 {isModelDropdownOpen
                ? 'border-vgd-yellow/70'
                : ''}"
            >
              {#if availableModels().find((m) => m.id === currentModel)}
                <span class="text-vgd-yellow truncate"
                ...
                {#each availableModels() as model}
                  ...
                  >
                    ...
                    {#if model.isImageModel}
                      <svg
                        class="w-3.5 h-3.5 flex-shrink-0 text-vgd-yellow"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                        stroke-width="2"
          ...
          <div class="flex items-center gap-4 text-xs font-mono flex-shrink-0">
            {#if currentTtft !== null}
              <span class="text-vgd-light-gray">
                ...
                <span class="text-vgd-yellow">{currentTtft.toFixed(1)}ms</span>
              </span>
            {/if}
            {#if currentTps !== null}
              <span class="text-vgd-light-gray">
                <span class="text-white/70">TPS</span>
                <span class="text-vgd-yellow">{currentTps.toFixed(1)}</span>
    ...
    <div class="flex items-start gap-2 sm:gap-3 py-3 px-3 sm:px-4">
      ...
      <!-- Terminal prompt -->
      <span class="text-vgd-yellow text-sm font-bold flex-shrink-0 leading-7"
        >▶</span
      >
    ...
    <!-- Bottom accent line -->
    <div
      class="absolute bottom-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-vgd-yellow/30 to-transparent"
    ></div>
  </div>

  {#if showHelperText}
    <p
      class="mt-2 sm:mt-3 text-center text-xs sm:text-xs text-vgd-light-gray tracking-[0.1em] sm:tracking-[0.15em] uppercase"
    >
      <kbd
        class="px-1 sm:px-1.5 py-0.5 rounded bg-vgd-medium-gray/30 text-vgd-light-gray border border-vgd-medium-gray/50"
        >ENTER</kbd
      >
