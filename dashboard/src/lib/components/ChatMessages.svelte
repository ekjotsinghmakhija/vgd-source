...
<div class="flex flex-col gap-4 sm:gap-6 {className}">
      ...
      >
        {#if message.role === "assistant"}
          ...
          <div class="flex items-center gap-1.5 sm:gap-2 mb-1.5 sm:mb-2">
            <div
              class="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-vgd-yellow rounded-full shadow-[0_0_10px_rgba(255,215,0,0.5)]"
            ></div>
            <span
              class="text-sm sm:text-xs text-vgd-yellow tracking-[0.15em] sm:tracking-[0.2em] uppercase font-medium"
              >VGD</span
            >
            <span
              class="text-xs sm:text-sm text-vgd-light-gray tracking-wider tabular-nums"
            ...
            {#if message.ttftMs || message.tps}
              <span class="text-xs text-vgd-light-gray/80 font-mono ml-2">
                {#if message.ttftMs}<span class="text-vgd-light-gray/50"
                    >TTFT</span
                  >
                  {message.ttftMs.toFixed(
                    0,
                  )}ms{/if}{#if message.ttftMs && message.tps}<span
                    class="text-vgd-light-gray/30 mx-1">•</span
                  >{/if}{#if message.tps}{message.tps.toFixed(1)}
                  <span class="text-vgd-light-gray/50">tok/s</span>{/if}
              </span>
            {/if}
          </div>
        {:else}
          ...
          >
            ...
            >
            <span
              class="text-sm sm:text-xs text-vgd-light-gray tracking-[0.1em] sm:tracking-[0.15em] uppercase"
              >QUERY</span
            >
            <div
              class="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-vgd-light-gray/50 rounded-full"
            ></div>
          </div>
        {/if}
                ...
                {#if message.attachments && message.attachments.length > 0}
                  <div class="flex flex-wrap gap-2 mb-3">
                    {#each message.attachments as attachment}
                      <div
                        class="flex items-center gap-2 bg-vgd-dark-gray/60 border border-vgd-yellow/20 rounded px-2 py-1 text-xs font-mono"
                      >
                        ...
                        {/if}
                        <span class="text-vgd-yellow" title={attachment.name}
            ...
            {:else}
              ...
              <div class="p-3 sm:p-4">
                {#if message.thinking && message.thinking.trim().length > 0}
                  <div
                    class="mb-3 rounded border border-vgd-yellow/20 bg-vgd-black/40"
                  >
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <span
                        class="text-[10px] tracking-[0.2em] text-vgd-light-gray/60 ml-4"
                      >
                    ...
                    {#each message.attachments.filter((a) => a.type === "generated-image") as attachment}
                      <div class="relative group/img inline-block">
                        <img
                          src={attachment.preview}
                          alt=""
                          class="max-w-full max-h-[512px] rounded-lg border border-vgd-yellow/20 shadow-lg shadow-black/20"
                        />
                        <!-- Button overlay -->
                        <div
                        ...
                        >
                          ...
                          <button
                            type="button"
                            class="p-2 rounded-lg bg-vgd-dark-gray/80 border border-vgd-yellow/30 text-vgd-yellow hover:bg-vgd-dark-gray hover:border-vgd-yellow/50 cursor-pointer"
                            onclick={() => {
                ...
                <div class="text-xs text-foreground">
                  {#if message.content === "Generating image..." || message.content === "Editing image..." || message.content?.startsWith("Generating...") || message.content?.startsWith("Editing...")}
                    <div class="flex items-center gap-3 text-vgd-yellow">
                      <div class="relative">
                        <div
                          class="w-8 h-8 border-2 border-vgd-yellow/30 border-t-vgd-yellow rounded-full animate-spin"
                        ></div>
                        <svg
                          class="absolute inset-0 w-8 h-8 p-1.5 text-vgd-yellow/60"
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                          stroke-width="2"
                  ...
                  {:else if message.content || (loading && !message.attachments?.some((a) => a.type === "generated-image"))}
                    ...
                    {#if loading && !message.content}
                      <span
                        class="inline-block w-2 h-4 bg-vgd-yellow/70 ml-1 cursor-blink"
                      ></span>
                    {/if}
                  {/if}
                </div>
  ...
  {#if messageList.length === 0}
    ...
    >
      <div
        class="w-12 h-12 sm:w-16 sm:h-16 border border-vgd-yellow/20 rounded-full flex items-center justify-center mb-3 sm:mb-4"
      >
        <div
          class="w-6 h-6 sm:w-8 sm:h-8 border border-vgd-yellow/40 rounded-full flex items-center justify-center"
        >
          <div
            class="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-vgd-yellow/60 rounded-full"
          ></div>
        </div>
      </div>
      <p
        class="text-xs sm:text-sm text-vgd-light-gray tracking-[0.15em] sm:tracking-[0.2em] uppercase"
      >
        AWAITING INPUT
      </p>
      <p class="text-sm sm:text-xs text-vgd-light-gray tracking-wider mt-1">
