...
<aside
  class="flex flex-col h-full bg-vgd-dark-gray border-r border-vgd-yellow/10 {className}"
>
  <!-- Header -->
  <div class="p-4">
    <button
  ...
  <div class="flex-1 overflow-y-auto">
        ...
        {#each filteredConversations as conversation (conversation.id)}
          ...
          <div class="px-2">
            {#if editingId === conversation.id}
              <!-- Edit mode -->
              <div
                class="p-2 bg-transparent border border-vgd-yellow/20 rounded mb-1"
              >
                <input
                  type="text"
    ...
    {:else}
      ...
      >
        <div
          class="w-12 h-12 border border-vgd-yellow/20 rounded-full flex items-center justify-center mb-3"
        >
          <svg
            class="w-6 h-6 text-vgd-yellow/40"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
