...
<div class="border-b border-vgd-medium-gray/30 px-3 py-2">
  <!-- Basic params row -->
  <div class="flex items-center gap-3 flex-wrap">
    <!-- Size -->
    <div class="flex items-center gap-1.5">
      <span class="text-xs text-vgd-light-gray uppercase tracking-wider"
        >SIZE:</span
      >
      <div class="relative">
        <button
          ...
          class="bg-vgd-medium-gray/50 border border-vgd-yellow/30 rounded pl-2 pr-6 py-1 text-xs font-mono text-vgd-yellow cursor-pointer transition-all duration-200 hover:border-vgd-yellow/50 focus:outline-none focus:border-vgd-yellow/70 {isSizeDropdownOpen
        ...
        >
          <svg
            class="w-3 h-3 text-vgd-yellow/60"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
      ...
      {#if isSizeDropdownOpen}
        ...
        <div
          class="fixed bg-vgd-dark-gray border border-vgd-yellow/30 rounded shadow-lg shadow-black/50 z-[9999] max-h-48 overflow-y-auto min-w-max"
    ...
    <div class="flex items-center gap-1.5">
      ...
      <div class="relative">
        <button
          ...
          class="bg-vgd-medium-gray/50 border border-vgd-yellow/30 rounded pl-2 pr-6 py-1 text-xs font-mono text-vgd-yellow cursor-pointer transition-all duration-200 hover:border-vgd-yellow/50 focus:outline-none focus:border-vgd-yellow/70 {isQualityDropdownOpen
            ? 'border-vgd-yellow/70'
            : ''}"
        >
    ...
    <div class="flex items-center gap-1.5">
      <span class="text-xs text-vgd-light-gray uppercase tracking-wider"
        >FORMAT:</span
      >
      <div class="flex rounded overflow-hidden border border-vgd-yellow/30">
        {#each outputFormatOptions as format}
          <button
            ...
            format
              ? 'bg-vgd-yellow text-vgd-black'
              : 'bg-vgd-medium-gray/50 text-vgd-light-gray hover:text-vgd-yellow'}"
          >
            {format}
          </button>
        {/each}
    ...
    >
      ...
      {#if hasAdvancedParams && !showAdvanced}
        <span class="w-1.5 h-1.5 rounded-full bg-vgd-yellow"></span>
      {/if}
    </button>
  </div>
  ...
  {#if showAdvanced}
    <div class="mt-3 pt-3 border-t border-vgd-medium-gray/20 space-y-3">
      ...
      <div class="flex items-center gap-4 flex-wrap">
        ...
        <div class="flex items-center gap-1.5">
          ...
          <input
            ...
            placeholder="Random"
            class="w-24 bg-vgd-medium-gray/50 border border-vgd-yellow/30 rounded px-2 py-1 text-xs font-mono text-vgd-yellow placeholder:text-vgd-light-gray/50 transition-all duration-200 hover:border-vgd-yellow/50 focus:outline-none focus:border-vgd-yellow/70"
          />
        </div>
        ...
        <div class="flex items-center gap-1.5 flex-1 min-w-[200px]">
          <span
            class="text-xs text-vgd-light-gray uppercase tracking-wider whitespace-nowrap"
            >STEPS:</span
          >
          <div class="flex items-center gap-2 flex-1">
            <input
              ...
              class="flex-1 h-1 bg-vgd-medium-gray/50 rounded appearance-none cursor-pointer accent-vgd-yellow"
            />
            <span class="text-xs font-mono text-vgd-yellow w-8 text-right">
      ...
      <div class="flex flex-col gap-1.5">
        ...
        <textarea
          ...
          rows={2}
          class="w-full bg-vgd-medium-gray/50 border border-vgd-yellow/30 rounded px-2 py-1.5 text-xs font-mono text-vgd-yellow placeholder:text-vgd-light-gray/50 resize-none transition-all duration-200 hover:border-vgd-yellow/50 focus:outline-none focus:border-vgd-yellow/70"
        ></textarea>
      </div>
      ...
      <div class="flex justify-end pt-1">
        <button
          type="button"
          onclick={handleReset}
          class="text-xs font-mono tracking-wider uppercase text-vgd-light-gray hover:text-vgd-yellow transition-colors duration-200"
        >
          RESET TO DEFAULTS
        </button>
      </div>
