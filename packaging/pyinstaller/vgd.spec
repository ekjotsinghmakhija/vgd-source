...
ENTRYPOINT = SOURCE_ROOT / "vgd" / "__main__.py"
DASHBOARD_DIR = PROJECT_ROOT / "dashboard" / "build"
VGD_SHARED_MODELS_DIR = SOURCE_ROOT / "vgd" / "shared" / "models"

if not ENTRYPOINT.is_file():
    raise SystemExit(f"Unable to locate Vgd entrypoint: {ENTRYPOINT}")

if not DASHBOARD_DIR.is_dir():
    raise SystemExit(f"Dashboard assets are missing: {DASHBOARD_DIR}")

if not VGD_SHARED_MODELS_DIR.is_dir():
    raise SystemExit(f"Shared model assets are missing: {VGD_SHARED_MODELS_DIR}")
...
DATAS: list[tuple[str, str]] = [
    ...
    (str(VGD_SHARED_MODELS_DIR), "vgd/shared/models"),
]
