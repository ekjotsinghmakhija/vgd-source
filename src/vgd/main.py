...
import vgd.routing.topics as topics
from vgd.master.api import API  # TODO: should API be in master?
from vgd.master.main import Master
from vgd.routing.router import Router, get_node_id_keypair
from vgd.shared.constants import VGD_LOG
from vgd.shared.election import Election, ElectionResult
from vgd.shared.logging import logger_cleanup, logger_setup
from vgd.shared.types.common import NodeId, SessionId
from vgd.utils.channels import Receiver, channel
from vgd.utils.pydantic_ext import CamelCaseModel
from vgd.worker.download.impl_shard_downloader import vgd_shard_downloader
from vgd.worker.main import Worker
...
class Node:
    ...
    async def create(cls, args: "Args") -> "Self":
        ...
        election = Election(
            node_id,
            # If someone manages to assemble 1 MILLION devices into an vgd cluster then. well done. good job champ.
...
def main():
    ...
    logger_setup(VGD_LOG, args.verbosity)
    logger.info("Starting VGD")
    logger.info(f"VGD_LIBP2P_NAMESPACE: {os.getenv('VGD_LIBP2P_NAMESPACE')}")

    # Set FAST_SYNCH override env var for runner subprocesses
    if args.fast_synch is True:
        os.environ["VGD_FAST_SYNCH"] = "on"
        logger.info("FAST_SYNCH forced ON")
    elif args.fast_synch is False:
        os.environ["VGD_FAST_SYNCH"] = "off"
        logger.info("FAST_SYNCH forced OFF")

    node = anyio.run(Node.create, args)
    anyio.run(node.run)
    logger.info("VGD Shutdown complete")
    logger_cleanup()
...
class Args(CamelCaseModel):
    ...
    @classmethod
    def parse(cls) -> Self:
        parser = argparse.ArgumentParser(prog="VGD")
        default_verbosity = 0
        parser.add_argument(
            "-q",
            "--quiet",
