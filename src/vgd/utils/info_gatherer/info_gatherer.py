...
from vgd.shared.constants import VGD_CONFIG_FILE
from vgd.shared.types.memory import Memory
from vgd.shared.types.profiling import (
    MemoryUsage,
    NetworkInterfaceInfo,
)
from vgd.shared.types.thunderbolt import (
    ThunderboltConnection,
    ThunderboltConnectivity,
    ThunderboltIdentifier,
)
from vgd.utils.channels import Sender
from vgd.utils.pydantic_ext import TaggedModel
...
class NodeConfig(TaggedModel):
    """Node configuration from VGD_CONFIG_FILE, reloaded from the file only at startup. Other changes should come in through the API and propagate from there"""

    @classmethod
    async def gather(cls) -> Self | None:
        cfg_file = anyio.Path(VGD_CONFIG_FILE)
