...
from vgd.shared.types.worker.downloads import DownloadProgressData
from vgd.shared.types.worker.shards import ShardMetadata
from vgd.worker.download.huggingface_utils import (
    filter_repo_objects,
    get_allow_patterns,
    get_auth_headers,
    get_hf_endpoint,
...
def build_model_path(model_id: str) -> DirectoryPath:
    return VGD_MODELS_DIR / model_id.replace("/", "--")
...
async def ensure_models_dir() -> Path:
    await aios.makedirs(VGD_MODELS_DIR, exist_ok=True)
    return VGD_MODELS_DIR
