...
from vgd.shared.constants import VGD_MAX_CHUNK_SIZE
from vgd.shared.models.model_cards import ModelId, ModelTask
from vgd.shared.types.api import ChatCompletionMessageText, ImageGenerationStats
from vgd.shared.types.chunks import ErrorChunk, ImageChunk, TokenChunk, ToolCallChunk
from vgd.shared.types.common import CommandId
from vgd.shared.types.events import (
    ChunkGenerated,
    Event,
    RunnerStatusUpdated,
    TaskAcknowledged,
...
from vgd.shared.types.worker.instances import BoundInstance
from vgd.shared.types.worker.runner_response import (
    GenerationResponse,
...
from vgd.shared.types.worker.runners import (
    RunnerConnected,
    RunnerConnecting,
    RunnerFailed,
    RunnerIdle,
...
from vgd.shared.types.worker.shards import ShardMetadata
from vgd.utils.channels import MpReceiver, MpSender
from vgd.worker.engines.image import (
...
from vgd.worker.engines.mlx.generator.generate import mlx_generate, warmup_inference
from vgd.worker.engines.mlx.utils_mlx import (
    apply_chat_template,
...
from vgd.worker.runner.bootstrap import logger
...
def _send_image_chunk(
    ...
    data_chunks = [
        encoded_data[i : i + VGD_MAX_CHUNK_SIZE]
        for i in range(0, len(encoded_data), VGD_MAX_CHUNK_SIZE)
    ]
...
VGD_RUNNER_MUST_FAIL = "VGD RUNNER MUST FAIL"
VGD_RUNNER_MUST_OOM = "VGD RUNNER MUST OOM"
VGD_RUNNER_MUST_TIMEOUT = "VGD RUNNER MUST TIMEOUT"
...
def _check_for_debug_prompts(
    ...
    if VGD_RUNNER_MUST_FAIL in prompt:
        logger.info("raising exception")
        raise Exception("Artificial runner exception - for testing purposes only.")
    if VGD_RUNNER_MUST_OOM in prompt:
        mlx_force_oom()
    if VGD_RUNNER_MUST_TIMEOUT in prompt:
        time.sleep(100)
